...
export const getHashAndUriFromNFT =  (
  ...
  return { imageHash, audioHash, audioUrl, imageUrl };
};
...
export function myImageLoader({ src, width, quality, height }: { src: string; width?: number; quality?: number, height?: number }) {

  return `${cdnUrl}${src}?quality=${quality || 75}${width ?`&width=${width}`:""}${height ?`&height=${height}`:""}`
}

export const ipfsUrl = (hash: string | null | undefined, quality = 90, path?: string, pinnedStatus?: string, width?:number, height?: number) => {
  if (!hash) return;
...
export const getCreatorNames = (track: SongDetailType) => {
  const creatorNames: { name: string; walletAddress: string }[] = [];
  track?.creators?.forEach((creator) => {
    creatorNames.push({
      name: creator.firstName || creator.walletAddress.slice(0, 3),
